...
module.exports = class Relaxation {
  ...
  static async patchRelaxation(technique, timing) {
    const { rows } = await pool.query(
      'UPDATE relaxations SET technique = ($2) WHERE id = ($1) RETURNING *',
      [technique, timing]
    );
    return new Relaxation(rows[0]);
  }

  static async deleteRelaxation(id) {
    const { rows } = await pool.query(
      'DELETE FROM relaxations WHERE id = ($1) RETURNING *',
      [id]
    );
    return new Relaxation(rows[0]);
  }
